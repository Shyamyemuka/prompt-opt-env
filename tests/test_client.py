...
def test_client_import():
    ...
    assert issubclass(PromptOptEnvEnv, EnvClient)


def test_client_step_payload():
    """_step_payload correctly serialises PromptAction to action_id dict."""
    from prompt_opt_env.client import PromptOptEnvEnv
    ...
    for action_id in range(6):
        payload = client._step_payload(PromptAction(action_id=action_id))
...
def test_client_parse_result_full_observation():
    ...
    assert obs.current_token_count == 4
    assert obs.previous_token_count == 6
    assert obs.token_budget == 80
    assert obs.tokens_remaining == 76
    assert obs.token_overhead == -2
