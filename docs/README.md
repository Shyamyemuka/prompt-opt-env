# PromptRL: Cost-Aware Task-Adaptive Prompt Optimization via RL
...
PromptRL frames this as **constrained RL**: the agent must learn *which prompt improvements are worth their token cost* and *when to stop adding tokens*. The reward signal explicitly balances two competing objectives:
...
# 5. Or connect and run an episode directly
python -c "
...
async def main():
    async with PromptRLEnv(base_url='ws://localhost:8000') as env:
        ...
        print(f'Budget: {obs.token_budget} tokens | Initial: {obs.current_token_count} tokens')
        while not result.done:
            result = await env.step(PromptAction(action_id=random.randint(0, 5)))
            obs = result.observation
            print(f'Step {obs.step_count}: {obs.info[\"action_applied\"]} tokens={obs.current_token_count}/{obs.token_budget} reward={result.reward:.4f}')
        print(f'Done: {obs.info[\"termination_reason\"]} | Final score: {obs.current_score:.4f}')
...
async with PromptRLEnv(base_url="wss://{username}-prompt-rl.hf.space") as env:
    result = await env.reset()
    # run episode...
```
...
raw_reward     = quality_delta - α × token_overhead          # α = TOKEN_PENALTY_ALPHA = 0.02
...
**Why α = 0.02**: Adding 10 tokens costs 0.2 reward. A typical quality improvement of +0.10 ROUGE-L is worth +0.10. So adding more than 5 tokens is only justified if quality improves by more than 0.10 — a calibrated trade-off that creates genuine learning tension.
...
async def collect_episode(env_url: str) -> list[dict]:
    ...
    trajectory = []
    async with PromptRLEnv(base_url=env_url) as env:
        result = await env.reset()
        while not result.done:
...
├── .env.example
└── prompt_rl/
    ├── models.py                     # PromptAction (0–5), PromptObservation (all token fields)
    ├── client.py                     # PromptRLEnv WebSocket client
    ├── openenv.yaml                  # OpenEnv manifest
    ├── pyproject.toml
    ├── server/
    │   ├── app.py
    │   ├── prompt_rl_environment.py  # Core RL logic (STOP, budget, cost-aware reward)
...
- Adaptive α: cost penalty increases as tokens_remaining decreases
